...
{
    public class ReferralUnlockCampaignService
    {
        private readonly ILogger<ReferralUnlockCampaignService> _logger;
        ...
        public ReferralUnlockCampaignService(
            ILogger<ReferralUnlockCampaignService> logger,
        ...
        {
            _logger.LogInformation($"======================Referal unlock campaign ProcessNonCumulativeCampaigns Start {preFix} ===============================");
            ...
            MatchedCampaign _setCamTyp(EarnCampaign campaign, bool isDirect, bool isLock, bool isUnlock, bool isReferralReward, int qualifiedTransactionCount)
            {
                ...
                return new MatchedCampaign
                {
                    ...
                    NoOfQualifiedTransactionsCount = qualifiedTransactionCount,
            ...
            foreach (var campaign in nonCumulativeCampaigns)
            {
                ...
                // Spend event
                var _offerType = campaign.OfferType; // General, Triple Reward
                ...
                if (string.Equals(_offerType, OfferTypeEnum.GENERAL_OFFERS, _strOrd))
                {
                    ...
                    if (string.Equals(_txnEventId, EventEnum.SPEND, _strOrd) && isReferral)
                    {
                        var flag = true;
                        ...
                        if (isSingle)
                        {
                            ...
                            if (flag && (IsP2P(configuredPaymentCategories, transctionPaymentCategory) || IsP2MOFF(configuredPaymentCategories, transctionPaymentCategory) || IsP2MON(configuredPaymentCategories, transctionPaymentCategory)))
                            {
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.GENERIC_ACTIVITY, _strOrd) && isReferral)
                    {
                        if (string.Equals(_eventType, EventTypeEnum.LENDING, _strOrd))
                        {
                            if (campaign.RewardCriteria.WithLockUnlock.UnlockEvent.UnlockBasedOn == "event")
                            {
                                var _childEventCode = unlockEvent?.Event?.GetGenericActivity()?.ChildEventCode;
                        ...
                        if (string.Equals(childEventCode, _unlockChildEventCode, _strOrd))
                        {
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.WALLET_LOAD, _strOrd) && isReferral)
                    {
                        ...
                        if (_flgWltMtch)
                        {
                            var tempTxns = _processorService.GetTempTransactions(_mobileNumber, campaign).FirstOrDefault();
                            if (tempTxns?.IsReferEarn != 1)
                            {
                                _logger.LogInformation($"{preFix} tempTxn.IsReferEarn:{tempTxns?.IsReferEarn} Referrar unlock");
                                continue;
                            }
                            matchedCampaigns = matchedCampaigns.Append(_setCamTyp(campaign, false, false, true, isReferralReward, transactionCount));
                            //continue;
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.UPI_LITE, _strOrd) && isReferral)
                    {
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.CLEAR_BOUNCE_EMI, _strOrd) && isReferral)
                    {
                        ...
                        {
                            var tempTx = _processorService.GetTempTransactions(_mobileNumber, campaign).FirstOrDefault();
                            if (tempTx?.IsReferEarn != 1)
                            {
                                _logger.LogInformation($"{preFix} IsReferEarn:{tempTx?.IsReferEarn} Referrar unlock");
                                continue;
                            }
                            matchedCampaigns = matchedCampaigns.Append(_setCamTyp(campaign, false, false, true, isReferralReward, transactionCount));
                            //continue;
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.KYC_COMPLETION, _strOrd) && isReferral)
                    {
                        ...
                        {
                            var tempTxna = _processorService.GetTempTransactions(_mobileNumber, campaign).FirstOrDefault();
                            if (tempTxna?.IsReferEarn != 1)
                            {
                                _logger.LogInformation($"{preFix} tempTxn.IsReferEarn:{tempTxna?.IsReferEarn} Referrar unlock");
                                continue;
                            }
                            matchedCampaigns = matchedCampaigns.Append(_setCamTyp(campaign, false, false, true, isReferralReward, transactionCount));
                            //continue;
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.DO_ISSUE, _strOrd) && isReferral)
                    {
                        if (string.Equals(campaign.RewardCriteria.RewardIssuance, RewardIssuanceEnum.WITH_LOCK_UNLOCK, _strOrd) && campaign.RewardCriteria.WithLockUnlock.UnlockEvent.UnlockBasedOn == "event")
                        {
                    ...
                    else if (string.Equals(_txnEventId, EventEnum.DI_COMPLETED, _strOrd) && isReferral)
                    {
                        if (string.Equals(campaign.RewardCriteria.RewardIssuance, RewardIssuanceEnum.WITH_LOCK_UNLOCK, _strOrd) && campaign.RewardCriteria.WithLockUnlock.UnlockEvent.UnlockBasedOn == "event")
                        {
            ...
            _logger.LogInformation("{PreFix} =================================== Referral Unlock campaign ProcessNonCumulativeCampaigns End ===============================", preFix);
            return transaction;
